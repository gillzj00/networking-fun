...
**Audit scope:** read-only review of live AWS state in mgmt `174230265051` and dev `065882629560` after Slice 1 (PR #15) landed the multi-account landing zone, cross-checked against `bootstrap/`, `docs/account-setup/policies/`, and the runbook.
...
**Where:** dev `065882629560`, role `gha-terraform`. Trust `Condition.StringLike.token.actions.githubusercontent.com:sub = "repo:gillzj00/networking-fun:*"`.
...
**Where:** mgmt `174230265051`, bucket `aws-cloudtrail-org-174230265051-us-east-2`.
...
```json
{
  ...
  "Action": "s3:*",
  "Resource": [
    "arn:aws:s3:::aws-cloudtrail-org-174230265051-us-east-2",
    "arn:aws:s3:::aws-cloudtrail-org-174230265051-us-east-2/*"
  ],
...
**Where:** dev `065882629560`, role `OrganizationAccountAccessRole`, trust = `arn:aws:iam::174230265051:root`, `AdministratorAccess`, `MaxSessionDuration=3600`.
...
**Where:** dev bucket `tfstate-networking-fun-065882629560`; `get-bucket-policy` → `NoSuchBucketPolicy`.
...
```
Re-version via `aws organizations update-policy --policy-id p-foeniyx6 --content file://...`.
...
**Where:** `bootstrap/main.tf:3` — `bucket = "tfstate-networking-fun-065882629560"`. **Not** on the runbook's pre-public-flip redact list at `docs/account-setup/README.md:35-39`.
...
**Fix:** Extend list, re-version via `aws organizations update-policy --policy-id p-95zla5mytk --content file://...`.

### F-14 [LOW] Budget alarms single-channel email
**Where:** `networking-fun-dev-25usd` — all 4 subscribers EMAIL to `zachary.gill@hotmail.com`.
...
**Where:** both PS in `arn:aws:sso:::instance/ssoins-79070b7972f6e5ba`.
...
- mgmt account `174230265051` lives at root, outside Workloads SCP — by design; mgmt has 9 pre-existing buckets + default VPC + IGW that are not covered by project guardrails.
