...
**Purpose.** Convert the existing single management account (`174230265051`) into an
...
| Field | Value |
|---|---|
| Management account ID | `174230265051` |
| Region | `us-east-2` |
...
| Member account ID | `065882629560` (created 2026-05-25) |
| Org ID | `o-md68aqrkqu` |
| Root ID | `r-2vqa` |
| Workloads OU ID | `ou-2vqa-ucilm6mg` |
| Budget cap | `$25` / month |
| Lab TTL | `4h` |
...
- [x] Done (2026-05-25) — SCP + TAG_POLICY enabled on root `r-2vqa`; Workloads OU `ou-2vqa-ucilm6mg` created
...
aws s3api create-bucket \
  --bucket aws-cloudtrail-org-174230265051-us-east-2 \
  --region us-east-2 \
...
aws cloudtrail create-trail \
  --name org-trail \
  --s3-bucket-name aws-cloudtrail-org-174230265051-us-east-2 \
...
- [x] Done (2026-05-25) — account `065882629560` created, moved into Workloads OU.
...
- [x] SCP `workloads-baseline` (`p-foeniyx6`) attached to Workloads OU (2026-05-25)
- [x] Tag policy `require-standard-tags` (`p-95zla5mytk`) attached to Workloads OU (2026-05-25)
...
1. Create + attach SCP:
   ...
   ```bash
   WORKLOADS_OU_ID=ou-2vqa-ucilm6mg
...
cat > /tmp/budget.json <<'JSON'
{
  ...
  "TimeUnit": "MONTHLY",
  "BudgetType": "COST",
  "CostFilters": { "LinkedAccount": ["065882629560"] },
  "CostTypes": {
...
aws budgets create-budget \
  --account-id 174230265051 \
  --budget file:///tmp/budget.json \
  --notifications-with-subscribers file:///tmp/notifications.json \
  --profile default

# Verify
aws budgets describe-budget --account-id 174230265051 \
  --budget-name networking-fun-dev-25usd --profile default
aws budgets describe-notifications-for-budget --account-id 174230265051 \
  --budget-name networking-fun-dev-25usd --profile default
```
...
- [x] Done (2026-05-25)
- SSO instance: `arn:aws:sso:::instance/ssoins-79070b7972f6e5ba`
- Identity Store: `d-9267e73b96`
- User: `zach-sso` (`3801e340-3091-709c-240e-b2c4fcff0ea3`)
- `NetworkingFunDevAdmin`: `arn:aws:sso:::permissionSet/ssoins-79070b7972f6e5ba/ps-7907d5f5dd52a5a6` (AdministratorAccess attached)
- `NetworkingFunDevReadOnly`: `arn:aws:sso:::permissionSet/ssoins-79070b7972f6e5ba/ps-79079cbccaa5b73b` (ReadOnlyAccess attached)
- Both assigned to user on dev account `065882629560`
...
- [x] Done (2026-05-25) — profiles `networking-fun-dev` and `networking-fun-dev-ro` added to `~/.aws/config`; existing `zach-sso` SSO session covered both; region-lock test confirmed (us-west-2 deny via SCP `p-foeniyx6`, us-east-2 success).
...
# Account: 065882629560
# Arn:     arn:aws:sts::065882629560:assumed-role/AWSReservedSSO_NetworkingFunDevAdmin_*/<your-sso-user>
```
...
- State bucket: `tfstate-networking-fun-065882629560` (versioned, SSE-S3, PAB on, 90d non-current expiration)
- OIDC provider: `arn:aws:iam::065882629560:oidc-provider/token.actions.githubusercontent.com`
- GHA role: `arn:aws:iam::065882629560:role/gha-terraform` (trusts `repo:gillzj00/networking-fun:*`, 4h sessions, AdministratorAccess)
...
`aws sts get-caller-identity`. If it returns `arn:aws:sts::065882629560:assumed-role/gha-terraform/...`
you're done.
